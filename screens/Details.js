...
const DetailsHeader = ({ data, navigation }) => (
  <View style={{ width: '100%', height: 373 }}>
    <Image                         // background image with respectative image with the place a bid
      source={data.image}
      resizeMode="cover"
      style={{ width: '100%', height: '100%' }}
    />
    <CircleButton                   //top left < icon fixed with image
      imageUrl={assets.left}
      handlePress={() => navigation.goBack()}
      left={15}
      top={StatusBar.currentHeight + 10}      
    />
    <CircleButton                   // top right heart icon fixed with image
